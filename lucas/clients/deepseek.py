...
class DeepseekClient:
    def __init__(self, tokens_rate=40000, period=60, max_tokens=4096, model='deepseek-chat', cache=''):
        ...
        self.logger = ConversationLogger('deepseek')
    ...
    def send(self, message, toolset=None, max_iterations=10):
        ...
        for i in range(max_iterations):
            ...
            bump('deepseek_prompt_tokens', data['usage']['prompt_tokens'])
            bump('deepseek_completion_tokens', data['usage']['completion_tokens'])
            bump('deepseek_total_tokens', data['usage']['total_tokens'])
